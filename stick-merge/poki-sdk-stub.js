(function() {
  ...
  var _pokiStub = {
    ...
    gameplayStart: () => {
      console.log("[poki-dl] gameplayStart invoked (stub)");
    },
    gameplayStop: () => {
      console.log("[poki-dl] gameplayStop invoked (stub)");
    },
    commercialBreak: _pp,
    rewardedBreak: function() { return Promise.resolve(true); },
    displayAd: function(data, size) {
      console.log("[poki-dl] displayAd invoked (stub)", data, size);
    },
    destroyAd: function() {
      console.log("[poki-dl] destroyAd invoked (stub)");
    },
    setDebug: _pn,
  ...
  console.log("[poki-dl] PokiSDK stub active (sealed by poki-ad-bridge-client.js when present)");
