(function() {
  ...
  // SiteLock 在 wasm 内通过 Application.absoluteURL -> framework.js 的
  // _JS_SystemInfo_GetDocumentURL -> document.URL 读取页面域名做校验；
  // 自有 CDN 域名不在允许列表(crazygames.com / crazygames.com.tr 等) → Code 3 锁死。
  // 这里把 document.URL 伪装成 crazygames.com（保留真实 pathname/search），
  // 但资源加载走 window.location.href(document.baseURI)，不受影响。
  var _realPath = (window.location.pathname || "/") + (window.location.search || "");
  var _spoofURL = "https://crazygames.com" + _realPath;
  try {
    Object.defineProperty(document, "URL", { get: function () { return _spoofURL; }, configurable: true });
    console.log("[cg-dbg] document.URL 已伪装为", _spoofURL);
  } catch (e) {
    console.log("[cg-dbg] document.URL 伪装失败:", e && e.message);
  }
  // candy 这版 SiteLock 还会直接读 window.location / document.location 的 hostname 做域名校验，
  // 仅伪装 document.URL 不够（puzzle.rabigame.fun 仍会被判 Code 3）。
  // 这里把 location 的 hostname/host/origin/href 伪装成 crazygames.com，路径完全保留，
  // 相对资源按 document.baseURI(真实) 解析，不受影响。
  try {
    ...
    var _fakeLoc = {
      ...
      host: "crazygames.com",
      origin: "https://crazygames.com",
      protocol: "https:",
      pathname: _realLoc.pathname || "/",
      port: "",
      search: _realLoc.search || "",
      hash: _realLoc.hash || "",
      assign: function () { return _realLoc.assign.apply(_realLoc, arguments); },
      replace: function () { return _realLoc.replace.apply(_realLoc, arguments); },
      reload: function () { return _realLoc.reload.apply(_realLoc, arguments); },
      toString: function () { return _spoofURL; }
    };
    try { Object.defineProperty(document, "location", { get: function () { return _fakeLoc; }, configurable: true }); } catch (e2) {}
    try { Object.defineProperty(window, "location", { get: function () { return _fakeLoc; }, configurable: true }); } catch (e2) {}
    console.log("[cg-dbg] window.location 已伪装为", _spoofURL);
  } catch (e) {
    console.log("[cg-dbg] window.location 伪装失败:", e && e.message);
  }
