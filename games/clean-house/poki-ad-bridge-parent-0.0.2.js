/**
 * Clean House — 父页面广告桥接（AdSense / adBreak）
 *
 * 与 cat-simulator 的 0.0.2 逻辑一致，仅替换 iframe id 与广告位 name：
 * 1. 打印 adBreak 的 breakStatus，便于区分「未填充 / 频次超限 / SDK 未就绪」
 * 2. 增加首响应超时兜底，避免 adBreakDone 不回调时游戏永久卡死；
 *    一旦 beforeAd 触发（广告真的开播）即取消超时，不打断正常播放
 *
 * 奖励只在 breakStatus === "viewed"（确实看完）时发放。广告未能投放时
 * 不发奖：否则装了拦截插件即可无限领奖，反而鼓励屏蔽广告。
 */
(function () {
  "use strict";

  var gameFrame = document.getElementById("chGameFrame");
  ...
  function showRewardedBreak() {
    return withAdStartTimeout("rewardedBreak", { rewardGranted: false }, function (settle, adStarted) {
      ...
      window.adBreak({
        type: "reward",
        name: "clean-house-reward",
        // reward 类型由 beforeReward 触发，beforeAd 通常不触发；此处不取消超时，
        // 避免误判「广告已开始」而提前取消兜底导致悬挂（与 happy-glass 一致）
        beforeAd: function () {
