(function() {
  ...
  var _pokiBase = {
    ...
    requestAd: _pp,
    cancelAd: _pn,
    // v3/unity-2020.js 会对这三个的返回值直接链式 .then()，
    // 必须返回 Promise，否则同步抛 TypeError 并打断 Unity 调用栈。
    // 本地无 Poki 账号，按「未登录」语义 reject，引擎已有 loginRejected /
    // getUserRejected / getTokenRejected 分支可正常接住。
    login: function() { return Promise.reject(new Error("poki_login_unavailable")); },
    getUser: function() { return Promise.reject(new Error("poki_user_unavailable")); },
    getToken: function() { return Promise.reject(new Error("poki_token_unavailable")); },
