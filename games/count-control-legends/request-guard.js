...
(function (global) {
  ...
  function logBlocked(url, kind, extra) {
    try {
      if (config.debug && extra != null) {
        console.warn("[RequestGuard] blocked", { url: url, kind: kind, extra: extra });
      } else {
        console.warn("[RequestGuard] blocked", toAbsoluteHref(url), "(" + kind + ")");
      }
    } catch (e) {}
  }
  ...
  function logRequestLine(kind, url, allowed, extra) {
    ...
    try {
      ...
      console.log("[RequestGuard] request", line);
    } catch (e) {}
  }
